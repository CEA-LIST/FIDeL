...
class ConformalPredictionTimeAndSpaceAll:
    def __init__(self, cfg, scores: torch.Tensor, heatmaps: torch.Tensor):
        ...
        # in this version, parameters are lists
        # the goal is to extract multiple scores at once with all parameters
        self.cfg = cfg
        self.alpha_time_default = cfg.threshold.alpha_time_default
        self.alphas_time = cfg.threshold.alpha_time
        self.alphas_space_default = cfg.threshold.alpha_space_default
        self.alphas_space = cfg.threshold.alpha_space
        self.split_rate_default = cfg.threshold.split_rate_default
        self.split_rates = cfg.threshold.split_rate
        self.patch_percentages = cfg.threshold.percentage_patch_detection
        ...
        # Mean µp
        self.mu_p = self.DcalA_space.mean(dim=0)

        # Step 3: scalA(t), scalA(p)
        # time
        self.scalA_t_v1 = self.compute_scalA_variant1_time()
        self.scalA_t_v2 = self.compute_scalA_variant2_time()
        # space
        self.scalA_p_v1 = self.compute_scalA_variant1_space()
        self.scalA_p_v2 = self.compute_scalA_variant2_space()

        # Step 4: compute h_time, h_space and thresholds ηₜ and ηp
        self.h_time_all = self.compute_quantile_h_time()
        self.h_space_all = self.compute_quantile_h_space()
        self.threshold_time_all = self.mu_t_all + self.h_time_all * self.scalA_t
        self.threshold_space_all = self.mu_p_all + self.h_space_all * self.scalA_p
